...
namespace Dreamteck.Splines.Editor
{
	public class ComputerSplitModule : ComputerEditorModule
	{
		...
		protected override void OnDrawScene()
		{
			...
			for (var i = 0; i < m_spline.pointCount; i++)
			{
				...
				if (SplineEditorHandles.CircleButton(pos,
					    ...
					    HandleUtility.GetHandleSize(pos) * 0.12f, 1f, m_spline.editorPathColor))
				{
					SplitAtPoint(i);
					change = true;
					break;
			...
			if (!change)
			{
				...
				Vector3 up = Vector3.Cross(editorCamera.transform.forward, projected.forward).normalized * size +
				             projected.position;
				Vector3 down = Vector3.Cross(projected.forward, editorCamera.transform.forward).normalized * size +
				               projected.position;
				...
				if (pointValue - pointIndex > m_spline.moveStep)
				{
					if (SplineEditorHandles.CircleButton(projected.position,
						    Quaternion.LookRotation(editorCamera.transform.position - projected.position),
						    HandleUtility.GetHandleSize(projected.position) * 0.12f, 1f, m_spline.editorPathColor))
					{
			...
			Handles.color = Color.white;
			DssplineDrawer.DrawSplineComputer(m_spline, 0.0, projected.percent);
		...
		private void SplitAtPercent(double percent)
		{
			...
			var splitPoints = new SplinePoint[m_spline.pointCount - lastPointIndex];
			float lerpPercent = Mathf.InverseLerp(lastPointIndex, nextPointIndex, pointValue);
			SplinePoint splitPoint = SplinePoint.Lerp(m_spline.GetPoint(lastPointIndex),
				m_spline.GetPoint(nextPointIndex), lerpPercent);
			splitPoint.SetPosition(m_spline.EvaluatePosition(percent));
			splitPoints[0] = splitPoint;
			for (var i = 1; i < splitPoints.Length; i++)
			{
				splitPoints[i] = m_spline.GetPoint(lastPointIndex + i);
			}
			...
			{
				splitPoints[i] = m_spline.GetPoint(i);
			}
		...
		{
			RecordUndo("Split Spline");
			var splitPoints = new SplinePoint[m_spline.pointCount - index];
			for (var i = 0; i < splitPoints.Length; i++)
			{
				splitPoints[i] = m_spline.GetPoint(index + i);
			}
			...
			{
				users[i].clipFrom =
					Dmath.InverseLerp((double)index / (m_spline.pointCount - 1), 1.0, users[i].clipFrom);
			...
			{
				users[i].clipFrom =
					Dmath.InverseLerp(0.0, (double)index / (m_spline.pointCount - 1), users[i].clipFrom);
		...
		private double ProjectMouse()
		{
			...
			float closestDistance =
				(Event.current.mousePosition - HandleUtility.WorldToGUIPoint(m_spline.GetPointPosition(0)))
				.sqrMagnitude;
