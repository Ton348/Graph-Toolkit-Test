	...
	public class SplinePositionerEditor : SplineTracerEditor
	{
		protected override void BodyGui()
		{
			...
			if (followTarget.objectReferenceValue == null)
			{
				...
				else
				{
					SerializedProperty percent =
						serializedObject.FindProperty("_result").FindPropertyRelative("percent");
					EditorGUILayout.BeginHorizontal();

					float pos = position.floatValue;
					pos = EditorGUILayout.Slider("Percent", pos, 0f, 1f);
			...
			EditorGUILayout.PropertyField(targetObject,
				new GUIContent("Target Object", "Which object to apply the transformations to."));
		...
		private void OnSetDistance(float distance)
		{
			...
			SerializedProperty position = serializedObject.FindProperty("_position");
			var positioner = (SplinePositioner)targets[longest];
			double travel = positioner.Travel(0.0, distance);
