...
namespace Dreamteck.Splines.Editor
{
	...
	{
		private SplineComputer m_addComp;
		private int m_addPoint;
		private int[] m_availablePoints;
		private bool m_connectionsOpen, m_settingsOpen;
		...
		public override void OnInspectorGUI()
		{
			...
			if (m_nodes.Length == 1)
			{
				if (m_addComp != null)
				{
					var pointNames = new string[m_availablePoints.Length];
					for (var i = 0; i < pointNames.Length; i++)
					{
						pointNames[i] = "Point " + (m_availablePoints[i] + 1);
					}

					if (m_availablePoints.Length > 0)
					{
						m_addPoint = EditorGUILayout.Popup("Link point", m_addPoint, pointNames);
					}
					else
					{
					...
					{
						if (node.HasConnection(m_addComp, m_availablePoints[m_addPoint]))
						{
							EditorGUILayout.HelpBox(
								"Connection already exists (" + m_addComp.name + "," + m_availablePoints[m_addPoint],
								MessageType.Error);
						}
				...
				else
				{
					...
					bool dragged = DreamteckEditorGui.DropArea(rect, out addComps);
			...
			if (connections.Length > 0)
			{
				for (var i = 0; i < connections.Length; i++)
				{
					...
					if (SplineEditorGui.EditorLayoutSelectableButton(new GUIContent("Swap Tangents"),
						    connections[i].spline.type == Spline.Type.Bezier, connections[i].invertTangents))
					{
		...
		private void SelectComputer(SplineComputer comp)
		{
			...
			if (m_addComp != null)
			{
				m_availablePoints = GetAvailablePoints(m_addComp);
			}
		...
		private void AddConnection(SplineComputer computer, int pointIndex)
		{
				    ...
				    "No"))
			{
				...
				if (connections.Length == 0)
				{
					...
					{
						case 1:
							SplinePoint point = m_addComp.GetPoint(pointIndex);
		...
		protected virtual void DuringSceneGui(SceneView current)
		{
			...
			for (var i = 0; i < m_availablePoints.Length; i++)
			{
				if (m_addComp.isClosed && i == points.Length - 1)
				{
					break;
				}

				Handles.Label(
					points[i].position + Camera.current.transform.up * HandleUtility.GetHandleSize(points[i].position) *
					0.3f, (i + 1).ToString());
				if (SplineEditorHandles.CircleButton(points[m_availablePoints[i]].position,
					    Quaternion.LookRotation(-camTransform.forward, camTransform.up),
					    HandleUtility.GetHandleSize(points[m_availablePoints[i]].position) * 0.1f, 2f,
					    m_addComp.editorPathColor))
				{
