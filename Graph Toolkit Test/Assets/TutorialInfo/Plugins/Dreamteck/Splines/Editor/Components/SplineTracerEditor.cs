...
namespace Dreamteck.Splines.Editor
{
	...
	public class SplineTracerEditor : SplineUserEditor
	{
		...
		private bool m_cameraFoldout;
		private TransformModuleEditor m_motionEditor;
		private Texture2D m_renderCanvas;
		private RenderTexture m_rt;
		private SplineTracer[] m_tracers = new SplineTracer[0];
		...
		protected override void OnEnable()
		{
			...
			for (var i = 0; i < m_tracers.Length; i++)
			{
		...
		private void CreateRt()
		{
			...
			m_rt = new RenderTexture(GetRtwidth(), GetRtheight(), 16, RenderTextureFormat.Default,
		...
		protected override void BodyGui()
		{
			...
			if (tracer.physicsMode == SplineTracer.PhysicsMode.Rigidbody)
			{
				         ...
				         tracer.updateMethod != SplineUser.UpdateMethod.FixedUpdate)
				{
					EditorGUILayout.HelpBox(
						"Switch to FixedUpdate mode to ensure smooth update for non-interpolated rigidbodies",
						MessageType.Warning);
				}
			}
			...
			if (m_tracers.Length == 1)
			{
				...
				if (m_cameraFoldout)
				{
					...
					if (m_cam != null)
					{
						if (m_rt == null || m_rt.width != GetRtwidth() || m_rt.height != GetRtheight())
						{
							CreateRt();
						}
