...
namespace Dreamteck.Splines.Editor
{
	...
	public class ObjectControllerEditor : SplineUserEditor
	{
		protected override void BodyGui()
		{
			...
			SerializedProperty retainPrefabInstancesInEditor =
				serializedObject.FindProperty("_retainPrefabInstancesInEditor");
			...
			if (objectMethod.intValue == (int)ObjectController.ObjectMethod.Instantiate)
			{
				...
				if (spawnMethod.intValue == (int)ObjectController.SpawnMethod.Count)
				{
					...
					if (hasObj)
					{
						EditorGUILayout.PropertyField(spawnCount, new GUIContent("Spawn Count"));
					}
					else
					{
					...
					if (lastSpawnCount != spawnCount.intValue)
					{
						objectsChanged = true;
					}
				}
		...
		{
			var serializedRule = new SerializedObject(customRule);
