...
namespace Dreamteck.Splines.Editor
{
	public class ComputerMergeModule : ComputerEditorModule
	{
		...
		private SplineComputer[] m_availableMergeComputers = new SplineComputer[0];
		...
		private void FindAvailableComputers()
		{
			...
			{
				if (found[i] != m_spline && !found[i].isClosed && m_spline.pointCount >= 2)
				{
		...
		protected override void OnDrawScene()
		{
			...
			for (var i = 0; i < m_availableMergeComputers.Length; i++)
			{
				...
				SplinePoint endPoint =
					m_availableMergeComputers[i].GetPoint(m_availableMergeComputers[i].pointCount - 1);
				Handles.color = m_availableMergeComputers[i].editorPathColor;

				if (SplineEditorHandles.CircleButton(startPoint.position,
					    Quaternion.LookRotation(editorCamera.transform.position - startPoint.position),
					    HandleUtility.GetHandleSize(startPoint.position) * 0.15f, 1f,
					    m_availableMergeComputers[i].editorPathColor))
				{
				...
				if (SplineEditorHandles.CircleButton(endPoint.position,
					    Quaternion.LookRotation(editorCamera.transform.position - endPoint.position),
