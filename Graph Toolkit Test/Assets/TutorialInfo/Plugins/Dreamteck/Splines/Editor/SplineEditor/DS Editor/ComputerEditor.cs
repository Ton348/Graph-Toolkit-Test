...
namespace Dreamteck.Splines.Editor
{
	...
	{
		private readonly SerializedProperty m_customNormalInterpolation;
		private readonly SerializedProperty m_customValueInterpolation;
		private readonly SerializedProperty m_knotParametrization;
		private readonly SerializedProperty m_linearAverageDirection;
		private readonly ComputerEditorModule[] m_modules = new ComputerEditorModule[0];
		private readonly SerializedProperty m_multithreaded;
		private readonly Toolbar m_operationsToolbar;
		private readonly SerializedProperty m_optimizeAngleThreshold;
		private readonly DreamteckSplinesEditor m_pathEditor;
		private readonly SerializedProperty m_sampleMode;
		private readonly SerializedProperty m_sampleRate;
		private readonly SerializedProperty m_space;
		private readonly SplineComputer m_spline;

		private readonly SerializedProperty m_splineProperty;
		private readonly SplineComputer[] m_splines = new SplineComputer[0];
		private readonly Toolbar m_transformToolbar;
		private readonly SerializedProperty m_type;
		private readonly SerializedProperty m_updateMode;
		private readonly Toolbar m_utilityToolbar;
		...
		private bool m_pathToolsFoldout, m_interpolationFoldout;


		public ComputerEditor(
			SplineComputer[] splines,
			...
			DreamteckSplinesEditor pathEditor) : base(serializedObject)
		{
			...
			GUIContent[] utilityContents = new GUIContent[m_modules.Length],
				utilityContentsSelected = new GUIContent[m_modules.Length];
			...
			m_utilityToolbar = new Toolbar(utilityContents, utilityContentsSelected, 35f);
			...
			m_transformToolbar = new Toolbar(transformContents, transformContentsSelected, 35f);
			...
			m_operationsToolbar = new Toolbar(operationContents, operationContentsSelected, 64f);
		...
		{
			base.Load();
			m_pathToolsFoldout = LoadBool("DreamteckSplinesEditor.pathToolsFoldout");
			m_interpolationFoldout = LoadBool("DreamteckSplinesEditor.interpolationFoldout");
			m_transformTool = LoadInt("DreamteckSplinesEditor.transformTool");
		}
		...
		public override void DrawInspector()
		{
			...
			EditorGUI.BeginChangeCheck();
			var lastType = (Spline.Type)m_type.intValue;
			...
			if (lastType == Spline.Type.CatmullRom && m_type.intValue == (int)Spline.Type.Bezier)
			{
				if (EditorUtility.DisplayDialog("Hermite to Bezier",
					    "Would you like to retain the Catmull Rom shape in Bezier mode?", "Yes", "No"))
				{
			...
			if (m_sampleMode.intValue == (int)SplineComputer.SampleMode.Optimized)
			{
				EditorGUILayout.PropertyField(m_optimizeAngleThreshold);
			}
			...
			if (m_updateMode.intValue == (int)SplineComputer.UpdateMode.None && Application.isPlaying)
			{
				if (GUILayout.Button("Rebuild"))
				{
					for (var i = 0; i < m_splines.Length; i++)
					{
						m_splines[i].RebuildImmediate(true, true);
					}
				}
			}
			...
			if (m_spline.type != Spline.Type.Linear)
			{
				EditorGUILayout.PropertyField(m_sampleRate, new GUIContent("Sample Rate"));
			}
			...
			{
				if (m_customValueInterpolation.animationCurveValue == null ||
				    m_customValueInterpolation.animationCurveValue.keys.Length == 0)
				{
					if (GUILayout.Button("Size & Color Interpolation"))
					{
						...
						for (var i = 0; i < m_splines.Length; i++)
						{
							m_splines[i].customValueInterpolation = curve;
						}
				...
				{
					EditorGUILayout.BeginHorizontal();
					EditorGUILayout.PropertyField(m_customValueInterpolation,
					...
					if (GUILayout.Button("x", GUILayout.MaxWidth(25)))
					{
						...
						for (var i = 0; i < m_splines.Length; i++)
						{
							m_splines[i].customValueInterpolation = null;
						}
				...
				if (m_customNormalInterpolation.animationCurveValue == null ||
				    m_customNormalInterpolation.animationCurveValue.keys.Length == 0)
				{
					if (GUILayout.Button("Normal Interpolation"))
					{
						...
						for (var i = 0; i < m_splines.Length; i++)
						{
							m_splines[i].customNormalInterpolation = curve;
						}
				...
				else
				{
					...
					if (GUILayout.Button("x", GUILayout.MaxWidth(25)))
					{
						...
						for (var i = 0; i < m_splines.Length; i++)
						{
							m_splines[i].customNormalInterpolation = null;
						}
			...
			if (EditorGUI.EndChangeCheck() || curveUpdate)
			{
				...
				for (var i = 1; i < m_splines.Length; i++)
				{
		...
		private void PerformOperation()
		{
			switch (m_operation)
			{
				...
				case 2:
				{
					...
					{
						for (var i = 0; i < m_pathEditor.points.Length; i++)
						{
		...
		public override void DrawScene(SceneView current)
		{
			...
			for (var i = 0; i < m_splines.Length; i++)
			{
				...
				if (drawPivot)
				{
					Transform trs = m_splines[i].transform;
			...
			if (m_pathEditor.currentModule == null)
			{
				...
				if (m_transformTool > 0)
				{
					...
					DreamteckEditorGui.Label(
						new Rect(screenPosition.x - 120 + m_spline.name.Length * 4, screenPosition.y, 120, 25),
						m_spline.name);
					Handles.EndGUI();
				}
			}
