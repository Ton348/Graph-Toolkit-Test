...
namespace Dreamteck.Splines.Editor
{
	public class PointMoveModule : PointTransformModule
	{
		...
		private Vector3 SurfaceMoveHandle(Vector3 inputPosition, float size = 0.2f)
		{
			...
			inputPosition = SplineEditorHandles.FreeMoveHandle(inputPosition,
		...
		protected override void OnDrawScene()
		{
			...
			if (lastPos != c)
			{
				...
				for (var i = 0; i < selectedPoints.Count; i++)
				{
					...
					if (snap)
					{
						points[selectedPoints[i]].SetPosition(SnapPoint(points[selectedPoints[i]].position));
					}
				}
			}
