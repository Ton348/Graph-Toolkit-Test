...
namespace Dreamteck.Splines.Editor
{
	public class SplineEditor : SplineEditorBase
	{
		...
		private readonly string m_customSplinePropertyName = "";
		...
		private float m_editLabelAlpha;
		private Vector2 m_editLabelPosition = Vector2.zero;
		private bool m_emptyClick;
		protected SplineSample m_evalResult = new();
		protected Vector2 m_lastClickPoint = Vector2.zero;
		private float m_lastEmptyClickTime;
		...
		protected List<PointOperation> m_pointOperations = new();
		private string[] m_pointOperationStrings = new string[0];
		protected bool m_pointToolsToggle;
		private int m_selectedPointOperation;
		...
		public List<int> selectedPoints = new();
		public EmptyHandler selectionChangeHandler;
		public SplineTravel travel;

		public SplineEditor(
			Matrix4x4 transformMatrix,
			SerializedObject splineHolder,
			string customSplinePropertyName) : base(splineHolder)
		{
		...
		protected virtual string editorName => "SplineEditor";

		public int moduleCount => m_modules.Length;
		...
		protected string splinePropertyName
		{
			get
			{
				if (m_customSplinePropertyName != "")
				{
					return m_customSplinePropertyName;
				}
		...
		private void Initialize(Matrix4x4 transformMatrix, SerializedObject splineHolder)
		{
			...
			foreach (string element in serializedPath)
			{
			...
			var moduleList = new List<PointModule>();
			...
			m_pointOperations.Add(new PointOperation
				{ name = "Distribute Evenly", action = delegate { DistributeEvenly(); } });
			m_pointOperations.Add(new PointOperation
				{ name = "Auto Bezier Tangents", action = delegate { AutoTangents(); } });
			m_pointOperations.Add(new PointOperation
				{ name = "Swap Bezier Tangents", action = delegate { SwapTangents(); } });
			m_pointOperations.Add(new PointOperation
				{ name = "Flip Bezier Tangents", action = delegate { FlipTangents(); } });
			m_pointOperations.Add(new PointOperation
				{ name = "Flip First Bezier Tangent", action = delegate { FlipFirstTangent(); } });
			m_pointOperations.Add(new PointOperation
				{ name = "Flip Seconds Bezier Tangent", action = delegate { FlipSecondTangent(); } });

			m_pointOperationStrings = new string[m_pointOperations.Count];
			for (var i = 0; i < m_pointOperations.Count; i++)
			{
		...
		{
			var p = new SplinePoint[points.Length];
		...
		public override void Destroy()
		{
			...
			{
				currentModule.Deselect();
			}

			if (lastEditorTool != Tool.None && Tools.current == Tool.None)
			{
				Tools.current = lastEditorTool;
			}
		}
			...
			if (editMode)
			{
				if (!gizmosEnabled)
				{
					EditorGUILayout.HelpBox(
						"Gizmos are disabled in the scene view. Enable Gizmos in the scene view for the spline editor to work.",
						MessageType.Error);
				}
		...
		public override void DrawScene(SceneView current)
		{
			...
			if (eventModule.mouseLeftDown)
			{
				m_lastClickPoint = e.mousePosition;
			}
			...
			if (eventModule.mouseLeftDown)
			{
				m_emptyClick = GUIUtility.hotControl == 0;
			}

			if (m_emptyClick)
			{
				if (eventModule.mouseLeft && !mainModule.isDragging &&
				    Vector2.Distance(m_lastClickPoint, e.mousePosition) >= mainModule.minimumRectSize &&
				    !eventModule.alt)
				{
			...
			{
				if (mainModule.isDragging)
				{
					mainModule.FinishDrag();
				}
				else
				{
		...
		protected virtual void PointMenu()
		{
			...
			var avgPoint = new SplinePoint(avgPos, avgPos);
			...
			for (var i = 0; i < selectedPoints.Count; i++)
			{
				points[selectedPoints[i]]
					.SetPosition(GetChangedVector(avgPos, avgPoint.position, points[selectedPoints[i]].position));
				points[selectedPoints[i]].normal =
					GetChangedVector(avgNormal, avgPoint.normal, points[selectedPoints[i]].normal);

				if (isBezier)
				{
					points[selectedPoints[i]]
						.SetTangentPosition(GetChangedVector(avgTan, avgPoint.tangent,
							points[selectedPoints[i]].tangent));
					points[selectedPoints[i]].SetTangent2Position(GetChangedVector(avgTan2, avgPoint.tangent2,
						points[selectedPoints[i]].tangent2));
				}
		...
		private Vector3 GetChangedVector(Vector3 oldVector, Vector3 newVector, Vector3 original)
		{
			if (!Mathf.Approximately(oldVector.x, newVector.x))
			{
				original.x = newVector.x;
			}

			if (!Mathf.Approximately(oldVector.y, newVector.y))
			{
				original.y = newVector.y;
			}

			if (!Mathf.Approximately(oldVector.z, newVector.z))
			{
				original.z = newVector.z;
			}

			return original;
		}

		private Vector3 TransformedPositionField(string title, Vector3 worldPoint)
		{
			Vector3 pos = worldPoint;
			if (editSpace == Space.Local)
			{
				pos = m_matrix.inverse.MultiplyPoint3x4(worldPoint);
			}
		...
		private Vector3 TransformedVectorField(string title, Vector3 worldPoint)
		{
			Vector3 vector = worldPoint;
			if (editSpace == Space.Local)
			{
				vector = m_matrix.inverse.MultiplyVector(worldPoint);
			}

			vector = EditorGUILayout.Vector3Field(title, vector);
			if (editSpace == Space.Local)
			{
				vector = m_matrix.MultiplyVector(vector);
			}

			return vector;
		}

		private Vector2 TransformedPositionField2D(string title, Vector3 worldPoint)
		{
		...
		public void FlatSelection(int axis)
		{
			...
			var flatTangent = false;
			var flatPosition = true;
			if (typeProperty.enumValueIndex == (int)Spline.Type.Bezier)
			{
				switch (EditorUtility.DisplayDialogComplex("Flat Bezier",
					        "How do you want to flat the selected Bezier points?", "Points Only", "Tangens Only",
					        "Everything"))
				{
					case 0:
						flatTangent = false;
			...
			if (flatPosition)
			{
				...
				for (var i = 0; i < selectedPoints.Count; i++)
				{
					...
					if (points[selectedPoints[i]].normal == Vector3.zero)
					{
		...
		{
			var mirrorTangents = false;
			if (typeProperty.enumValueIndex == (int)Spline.Type.Bezier)
			{
				if (EditorUtility.DisplayDialog("Mirror tangents", "Do you want to mirror the tangents too ?", "Yes",
					    "No"))
				{
					mirrorTangents = true;
				}
		...
		public void DistributeEvenly()
		{
			...
			var evalResult = new SplineSample();
		...
		public void AutoTangents()
		{
			...
			for (var i = 0; i < selectedPoints.Count; i++)
			{
				...
				{
					prevPos = points[index - 1].position;
				}

				if (index == points.Length - 1 && points.Length > 1)
				{
					forwardPos = points[points.Length - 1].position +
					             (points[points.Length - 1].position - points[points.Length - 2].position);
				}
				else
				{
					forwardPos = points[index + 1].position;
				}
