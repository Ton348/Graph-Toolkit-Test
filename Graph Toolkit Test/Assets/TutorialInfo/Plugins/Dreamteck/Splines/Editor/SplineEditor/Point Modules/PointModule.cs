...
namespace Dreamteck.Splines.Editor
{
	public class PointModule : EditorModule
	{
		...
		protected bool isClosed => m_editor.GetSplineClosed();

		protected int sampleRate => m_editor.GetSplineSampleRate();

		protected Spline.Type splineType => m_editor.GetSplineType();

		protected Color color => m_editor.drawColor;
		...
		{
			if (m_editor.undoHandler != null)
			{
				m_editor.undoHandler(title);
			}
		}

		protected override void Repaint()
		{
			if (m_editor.repaintHandler != null)
			{
				m_editor.repaintHandler();
			}
		}
		...
		public override void BeforeSceneDraw(SceneView current)
		{
			...
			if (m_movePivot)
			{
				SceneView.lastActiveSceneView.pivot =
					Vector3.Lerp(SceneView.lastActiveSceneView.pivot, m_idealPivot, 0.02f);
		...
		public virtual void DuplicateSelected()
		{
			...
			for (var i = 0; i < selectedPoints.Count; i++)
			{
				duplicated[index++] = points[selectedPoints[i]].CreateSplinePoint();
			}
			...
			if (duplicationDirection == Spline.Direction.Backward)
			{
				for (var i = 0; i < min; i++)
				{
					newPoints[i] = points[i].CreateSplinePoint();
				}
				...
				{
					newPoints[i + duplicated.Length] = points[i].CreateSplinePoint();
				}
			}
			else
			{
		...
		public void ClearSelection()
		{
			selectedPoints.Clear();
			Repaint();
			if (m_editor.selectionChangeHandler != null)
			{
				m_editor.selectionChangeHandler();
			}
		...
		{
			if (selectedPoints.Count == 1 && selectedPoints[0] == index)
			{
				return;
			}
