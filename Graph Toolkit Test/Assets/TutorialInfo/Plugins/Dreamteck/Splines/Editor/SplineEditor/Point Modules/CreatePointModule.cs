...
namespace Dreamteck.Splines.Editor
{
	public class CreatePointModule : PointModule
	{
		...
		protected SplineSample m_evalResult;
		...
		protected override void OnDrawScene()
		{
			...
			if (m_placementMode == PlacementMode.CameraPlane)
			{
				GetCreatePointOnPlane(-m_editorCamera.transform.forward,
					m_editorCamera.transform.position + m_editorCamera.transform.forward * offset, out m_createPoint);
			...
			if (m_placementMode == PlacementMode.Surface)
			{
				...
				if (Physics.Raycast(ray, out hit, Mathf.Infinity, surfaceLayerMask))
				{
					...
					SplineEditorHandles.DrawRectangle(m_createPoint,
						Quaternion.LookRotation(-m_editorCamera.transform.forward, m_editorCamera.transform.up),
						HandleUtility.GetHandleSize(m_createPoint) * 0.1f);
					Handles.color = Color.white;
		...
		{
			var newPoint = new SplinePoint(m_createPoint, m_createPoint);
			...
			else
			{
				...
				switch (normalMode)
				{
					...
					case NormalMode.LookAtCamera:
						points[index].normal =
							Vector3.Normalize(editorCamera.transform.position - points[index].position); break;
		...
		protected double ProjectScreenSpace(Vector2 screenPoint)
		{
			...
			double add = moveStep;
			if (splineType == Spline.Type.Linear)
			{
				add /= 2.0;
			}
		...
		private bool AxisGrid(Vector3 axis, Color color, out Vector3 origin)
		{
			...
			if (plane.Raycast(ray, out rayDistance))
			{
				...
				if (!m_editorCamera.orthographic && plane.Raycast(ray, out rayDistance))
				{
					distance = Vector3.Distance(m_editorCamera.transform.position + axis * offset, origin);
				}
				else if (m_editorCamera.orthographic)
				{
					distance = 2f * m_editorCamera.orthographicSize;
				}
		...
		private void UpdateVisualizer()
		{
			...
			{
				m_visualizer = new Spline(splineType);
			}
			...
			var newPoint = new SplinePoint(m_createPoint, m_createPoint, m_createNormal, 1f, Color.white);
			if (appendMode == AppendMode.End)
			{
			...
			{
				for (var i = 1; i < m_visualizer.points.Length; i++)
				{
