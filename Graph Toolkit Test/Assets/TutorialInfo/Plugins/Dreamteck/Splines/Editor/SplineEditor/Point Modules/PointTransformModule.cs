...
namespace Dreamteck.Splines.Editor
{
	public class PointTransformModule : PointModule
	{
		...
		public override void Reset()
		{
			...
			for (var i = 0; i < m_localPoints.Length; i++)
			{
				InverseTransformPoint(ref m_localPoints[i]);
			}
		}
			...
			bool size = false)
		{
			...
			if (normals)
			{
				point.normal = TransformDirection(point.normal).normalized;
			}
		...
		protected void InverseTransformPoint(
			ref SplinePoint point,
			bool normals = true,
			bool tangents = true,
			bool size = false)
		{
			...
			if (normals)
			{
				point.normal = InverseTransformDirection(point.normal).normalized;
			}
