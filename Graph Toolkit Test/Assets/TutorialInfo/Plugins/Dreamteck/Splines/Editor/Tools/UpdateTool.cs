	...
	public class UpdateTool : SplineTool
	{
		...
		private void UpdateNodes()
		{
			...
			for (var i = 0; i < nodes.Length; i++)
			{
				...
				m_updated += i + " - " + nodes[i].name + Environment.NewLine;
			}

			EditorUtility.ClearProgressBar();
			if (nodes.Length == 0)
			{
				m_updated += Environment.NewLine + "No active Nodes found in the scene.";
			}
		}
		...
		private void UpdateUsers()
		{
			...
			for (var i = 0; i < users.Length; i++)
			{
				...
				m_updated += i + " - " + users[i].name + Environment.NewLine;
			}

			EditorUtility.ClearProgressBar();
			if (users.Length == 0)
			{
				m_updated += Environment.NewLine + "No active SplineUsers found in the scene.";
			}
		}
		...
		private void UpdateMeshGenerators()
		{
			...
			if (users.Length == 0)
			{
				m_updated += Environment.NewLine + "No active MeshGenerators found in the scene.";
			}
		}
		...
		private void UpdateComputers()
		{
			...
			for (var i = 0; i < computers.Length; i++)
			{
				...
				m_updated += i + " - " + computers[i].name + Environment.NewLine;
			}

			EditorUtility.ClearProgressBar();
			if (computers.Length == 0)
			{
				m_updated += Environment.NewLine + "No active SplineComputers found in the scene.";
			}
		}
	}
}