...
namespace Dreamteck.Splines
{
	...
	{
		private readonly Dictionary<SplineComputer, SplinePoint[]> m_editSplines = new();
		private float m_maxSagDistance = 10f;
		private float m_minSagDistance;
		...
		public override void Draw(Rect windowRect)
		{
			...
			else
			{
				...
				Dictionary<SplineComputer, SplinePoint[]>.KeyCollection keys = m_editSplines.Keys;
				if (EditorGUI.EndChangeCheck())
				{
					SceneView.RepaintAll();
					foreach (SplineComputer key in keys)
					{
		...
		{
			SplinePoint current = m_editSplines[spline][index];
			if (index > 0)
			{
				SplinePoint previous = m_editSplines[spline][index - 1];
				Vector3 prevDirection = (previous.position - current.position) / 3f;
				float sagAmount = Mathf.InverseLerp(m_minSagDistance, m_maxSagDistance, prevDirection.magnitude) *
				                  m_sag;
			...
			{
				SplinePoint next = m_editSplines[spline][index + 1];
				Vector3 nextDirection = (next.position - current.position) / 3f;
				float sagAmount = Mathf.InverseLerp(m_minSagDistance, m_maxSagDistance, nextDirection.magnitude) *
				                  m_sag;
