...
namespace Dreamteck.Splines.Editor
{
	public class BakeTool : SplineTool
	{
		...
		private BakeGroup m_bakeGroup = BakeGroup.All;
		private bool m_copy;

		private DirectoryInfo m_dirInfo;
		private List<MeshGenerator> m_excluded = new();
		private BakeMeshWindow.SaveFormat m_format = BakeMeshWindow.SaveFormat.MeshAsset;
		private MeshGenerator[] m_found = new MeshGenerator[0];

		private bool m_isStatic = true;
		private bool m_permanent;
		private bool m_removeComputer;

		private string m_savePath = "";

		private Vector2 m_scroll1, m_scroll2;
		private List<MeshGenerator> m_selected = new();
		...
		public override void Draw(Rect windowRect)
		{
			...
			switch (m_format)
			{
				case BakeMeshWindow.SaveFormat.Scene:
					EditorGUILayout.HelpBox("Saves the mesh inside the scene", MessageType.Info); break;
			...
			if (!isCopy)
			{
				...
				if (m_permanent)
				{
					...
					{
						EditorGUILayout.HelpBox(
							"WARNING: Removing Spline Computers may cause other SplineUsers to stop working. Select this if you are sure that no other SplineUser uses the selected Spline Computers.",
							MessageType.Warning);
					}
				}
			}
			...
			if (GUILayout.Button("Bake"))
			{
				if (saveMesh)
				{
					...
					if (!Directory.Exists(m_savePath) || m_savePath == "")
					{
						EditorUtility.DisplayDialog("Save error",
							"Invalid save directory. Please select a valid save directory and try again", "OK");
						return;
					}

					if (m_format == BakeMeshWindow.SaveFormat.OBJ && !m_savePath.StartsWith(Application.dataPath) &&
					    !m_copy)
					{
						EditorUtility.DisplayDialog("Save error",
				...
				if (m_bakeGroup == BakeGroup.AllExcluding)
				{
					suff = "all excluding";
				}

				if (EditorUtility.DisplayDialog("Bake " + suff,
					    "This operation cannot be undone. Are you sure you want to bake the meshes?", "Yes", "No"))
				{
					switch (m_bakeGroup)
					{
		...
		private void BakeAll()
		{
			...
			for (var i = 0; i < m_found.Length; i++)
			{
		...
		{
			var availalbe = new List<MeshGenerator>(m_found);
