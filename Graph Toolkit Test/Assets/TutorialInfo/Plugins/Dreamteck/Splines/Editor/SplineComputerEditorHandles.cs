namespace Dreamteck.Splines.Editor
{
    ...
    {
        private static SplineSample s_evalResult = new SplineSample();
        ...
        public static bool Slider(SplineComputer spline, ref double percent, Color color, string text = "", SplineSliderGizmo gizmo = SplineSliderGizmo.Rectangle, float buttonSize = 1f)
        {
            ...
            Handles.DrawSolidDisc(s_evalResult.position, cam.transform.position - s_evalResult.position, size * 0.2f * buttonSize);
            ...
            var lookRotation = Quaternion.LookRotation(cam.transform.position - s_evalResult.position); 
            s_evalResult.position = SplineEditorHandles.FreeMoveHandle(s_evalResult.position, size * 0.2f * buttonSize, Vector3.zero, Handles.CircleHandleCap);
            if (s_evalResult.position != lastPos) percent = spline.Project(s_evalResult.position).percent;
            Handles.color = Color.white;
            return buttonClick;
        }
    }
