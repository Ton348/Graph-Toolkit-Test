	...
	public class Spiral : SplinePrimitive
	{
		...
		protected override void Generate()
		{
			...
			for (var i = 0; i <= iterations * 4; i++)
			{
				...
				{
					tangentRot =
						Quaternion.AngleAxis(Mathf.Lerp(0f, 90f * 0.16f * angleDirection, radiusDeltaPercent * percent),
							Vector3.up);
				}
				else
				{
					tangentRot =
						Quaternion.AngleAxis(
							Mathf.Lerp(0f, -90f * 0.16f * angleDirection, (1f - percent) * radiusDeltaPercent),
							Vector3.up);
				}

				if (clockwise)
				{
					m_points[i].tangent = m_points[i].position -
					                      (tangentRot * rot * Vector3.right * radius + Vector3.up * stretch / 4f) * 2 *
					                      (Mathf.Sqrt(2f) - 1f) / 3f;
				}
				else
				{
