...
namespace Dreamteck.Splines
{
	...
	{
		private static bool s_useModifier;
		private static ISampleModifier s_modifier;
		...
		private SplineSample m_workSample;
		...
		public int GetClippedSampleCount(double clipFrom, double clipTo, out int startIndex, out int endIndex)
		{
			...
			else
			{
				...
				if (clipToLerp > 0.0 && endIndex < samples.Length - 1)
				{
					endIndex++;
				}
			}
		...
		public void GetSamplingValues(double percent, out int sampleIndex, out double lerp)
		{
			...
			if (sampleMode == SplineComputer.SampleMode.Optimized)
			{
				...
				{
					lerp = Dmath.InverseLerp(samples[sampleIndex].percent, samples[sampleIndex + 1].percent,
						lerpPercent);
				}
			...
			double clipTo = 1.0)
		{
			...
			while (moved < distance)
			{
				...
				if (direction == Spline.Direction.Forward)
				{
					...
					if (samplesAreLooped && sampleIndex == maxIndex)
					{
						break;
					}
				...
				else
				{
					...
					if (samplesAreLooped && sampleIndex == minIndex)
					{
						break;
					}
		...
		/// <summary>
		///     Returns the percent from the spline at a given distance from the start point while applying a local
		///     <paramref name="offset" /> to each sample
			...
			double clipTo = 1.0)
		{
			...
			Vector3 lastPos = m_workSample.position + m_workSample.up * (offset.y * m_workSample.size) +
			                  m_workSample.right * (offset.x * m_workSample.size) +
			                  m_workSample.forward * (offset.z * m_workSample.size);
			...
			ISampleModifier modifier = null)
		{
			...
			int steps = (controlPointCount - 1) *
			            4; //Sampling four points per segment is enough to find the closest point range
			...
			if (backIndex < index && index < frontIndex)
			{
				...
				{
					SplineSample.Lerp(ref samples[backIndex], ref samples[index], backProjectDist / backLength,
						ref result);
					if (sampleMode == SplineComputer.SampleMode.Uniform)
					{
						result.percent = Dmath.Lerp(GetSamplePercent(backIndex), GetSamplePercent(index),
							backProjectDist / backLength);
					}
				}
				else
				{
					SplineSample.Lerp(ref samples[frontIndex], ref samples[index], frontProjectDist / frontLength,
						ref result);
					if (sampleMode == SplineComputer.SampleMode.Uniform)
					{
						result.percent = Dmath.Lerp(GetSamplePercent(frontIndex), GetSamplePercent(index),
							frontProjectDist / frontLength);
					}
				}
			}
			...
			if (from == 0.0 && to == 1.0 && result.percent < samples[1].percent) //Handle looped splines
			{
				...
				if ((position - projected).sqrMagnitude < (position - result.position).sqrMagnitude)
				{
					...
					if (sampleMode == SplineComputer.SampleMode.Uniform)
					{
						result.percent = Dmath.Lerp(GetSamplePercent(samples.Length - 1),
							GetSamplePercent(samples.Length - 2), l);
					}
				}
			}
		...
		/// <summary>
		///     Calculates the length between <paramref name="from" /> and <paramref name="to" /> with applied local offset to to
		///     the samples
		///     The offset is multiplied by the sample sizes
		/// </summary>
		...
		public float CalculateLengthWithOffset(Vector3 offset, double from = 0.0, double to = 1.0)
		{
			...
			for (int i = fromIndex + 1; i < toIndex; i++)
			{
				Vector3 newPos = samples[i].position + samples[i].up * (offset.y * samples[i].size) +
				                 samples[i].right * (offset.x * samples[i].size) +
				                 samples[i].forward * (offset.z * samples[i].size);
				length += Vector3.Distance(newPos, lastPos);
				lastPos = newPos;
			}

			Evaluate(to, ref m_workSample);
			m_workSample.position += m_workSample.up * (offset.y * m_workSample.size) +
			                         m_workSample.right * (offset.x * m_workSample.size) +
			                         m_workSample.forward * (offset.z * m_workSample.size);
