...
namespace Dreamteck.Splines
{
	...
	public class Spline
	{
		...
		private static Vector3[] s_p = new Vector3[4];
		private static Vector3 s_a1;
		private static Vector3 s_a2;
		private static Vector3 s_a3;
		private static Vector3 s_b1;
		private static Vector3 s_b2;
		private static float s_t1;
		private static float s_t2;
		private static float s_t3;
		...
		[SerializeField]
		[Range(0f, 1f)]
		private float m_knotParametrization;
		...
		/// </summary>
		public bool isClosed => m_closed && points.Length >= 3;
		...
		public int iterations
		{
			get
			{
				if (type == Type.Linear)
				{
					return m_closed ? points.Length + 1 : points.Length;
				}

				int segments = m_closed ? points.Length : points.Length - 1;
		...
		{
			get => m_knotParametrization;
			set => m_knotParametrization = Mathf.Clamp01(value);
		}
		...
		public double Project(Vector3 position, int subdivide = 4, double from = 0.0, double to = 1.0)
		{
			...
			if (m_closed && from == 0.0 && to == 1.0) //Handle looped splines
			{
				double closest = GetClosestPoint(subdivide, position, from, to,
					Mathf.RoundToInt(Mathf.Max(iterations / points.Length, 10)) * 5);
				if (closest < moveStep)
				{
					double nextClosest = GetClosestPoint(subdivide, position, 0.5, to,
						Mathf.RoundToInt(Mathf.Max(iterations / points.Length, 10)) * 5);
					if (Vector3.Distance(position, EvaluatePosition(nextClosest)) <
					    Vector3.Distance(position, EvaluatePosition(closest)))
					{
						return nextClosest;
					}
				}
			...
			return GetClosestPoint(subdivide, position, from, to,
				Mathf.RoundToInt(Mathf.Max(iterations / points.Length, 10)) * 5);
		}
		...
		public void Evaluate(double percent, ref SplineSample sample)
		{
			...
			var valueInterpolation = (float)getPercent;
			...
			var normalInterpolation = (float)getPercent;
			...
			if (type == Type.BSpline)
			{
				...
				else
				{
					...
					if (m_closed)
					{
						if (percent < step)
						{
							EvaluatePosition(1.0 - (step - percent), ref back);
						}
						else
						{
						...
						if (percent > 1.0 - step)
						{
							EvaluatePosition(step - (1.0 - percent), ref front);
						}
						else
						{
		...
		/// <param name="to">Target position [from-1]</param>
		/// <returns></returns>
		public void EvaluateUniform(
			ref SplineSample[] samples,
			ref double[] originalSamplePercents,
			double from = 0.0,
			double to = 1.0)
		{
			if (points.Length == 0)
		...
		public double Travel(double start, float distance, out float moved, Direction direction)
		{
			...
			int i = iterations - 1;
			int nextSampleIndex = direction == Direction.Forward ? Dmath.CeilInt(start * i) : Dmath.FloorInt(start * i);
			var lastDistance = 0f;
		...
		public void Break(int at)
		{
			...
			var previousPoints = new SplinePoint[points.Length];
			...
			switch (type)
			{
				...
				case Type.CatmullRom:
					for (var i = 0; i < points.Length; i++)
					{
						...
						if (m_knotParametrization > 0f)
						{
							ComputeCatPoints(i);
							points[i].SetTangent2Position(points[i].position +
							                              tangent.normalized * Vector3.Distance(s_p[0], s_p[2]) / 6f);
						}
						else
						{
		...
		private void CalculatePosition(ref Vector3 position, double percent, int pointIndex)
		{
			switch (type)
			{
				case Type.CatmullRom:
					...
					if (m_knotParametrization < 0.000001f)
					{
		...
		/// </summary>
		private void CalculatePositionAndTangent(
			double percent,
			int pointIndex,
		...
		private void CalculateLinearPosition(ref Vector3 position, double t, int i)
		{
			...
			position = Vector3.Lerp(s_p[1], s_p[2], (float)t);
		}
		...
		private void CalculateLinearTangent(ref Vector3 tangent, double t, int i)
		{
			...
			{
				tangent = Vector3.Slerp(s_p[1] - s_p[0], s_p[2] - s_p[1], 0.5f);
			}
			else
			{
				tangent = s_p[2] - s_p[1];
		...
		private void CalculateBsplinePosition(ref Vector3 position, double time, int i)
		{
			...
			if (points.Length > 1)
			{
				var tf = (float)Dmath.Clamp01(time);
				position = ((-s_p[0] + s_p[2]) / 2f
				            + tf * ((s_p[0] - 2f * s_p[1] + s_p[2]) / 2f
				                    + tf * (-s_p[0] + 3f * s_p[1] - 3f * s_p[2] + s_p[3]) / 6f)) * tf
				           + (s_p[0] + 4f * s_p[1] + s_p[2]) / 6f;
			}
		}
		...
		{
			const float t0 = 0f;
			s_t1 = GetInterval(s_p[0], s_p[1]);
			s_t2 = GetInterval(s_p[1], s_p[2]) + s_t1;
			s_t3 = GetInterval(s_p[2], s_p[3]) + s_t2;
			float tf = Mathf.LerpUnclamped(s_t1, s_t2, (float)t);

			s_a1 = (s_t1 - tf) / (s_t1 - t0) * s_p[0] + (tf - t0) / (s_t1 - t0) * s_p[1];
			s_a2 = (s_t2 - tf) / (s_t2 - s_t1) * s_p[1] + (tf - s_t1) / (s_t2 - s_t1) * s_p[2];
			s_a3 = (s_t3 - tf) / (s_t3 - s_t2) * s_p[2] + (tf - s_t2) / (s_t3 - s_t2) * s_p[3];

			s_b1 = (s_t2 - tf) / (s_t2 - t0) * s_a1 + (tf - t0) / (s_t2 - t0) * s_a2;
			s_b2 = (s_t3 - tf) / (s_t3 - s_t1) * s_a2 + (tf - s_t1) / (s_t3 - s_t1) * s_a3;


			float GetInterval(Vector3 a, Vector3 b)
			{
				return Mathf.Pow((a - b).sqrMagnitude, m_knotParametrization * 0.5f);
			}
		}
		...
		{
			float tf = Mathf.LerpUnclamped(s_t1, s_t2, (float)t);
			position = (s_t2 - tf) / (s_t2 - s_t1) * s_b1 + (tf - s_t1) / (s_t2 - s_t1) * s_b2;
		}

		private void CalculateCatmullRomTangent(double t, ref Vector3 tangent)
		{
			float tf = Mathf.LerpUnclamped(s_t1, s_t2, (float)t);
			Vector3 A1p = (s_p[1] - s_p[0]) / s_t1;
			Vector3 A2p = (s_p[2] - s_p[1]) / (s_t2 - s_t1);
			Vector3 A3p = (s_p[3] - s_p[2]) / (s_t3 - s_t2);

			Vector3 B1p = (s_a2 - s_a1) / s_t2 + (s_t2 - tf) / s_t2 * A1p + tf / s_t2 * A2p;
			Vector3 B2p = (s_a3 - s_a2) / (s_t3 - s_t1) + (s_t3 - tf) / (s_t3 - s_t1) * A2p +
			              (tf - s_t1) / (s_t3 - s_t1) * A3p;

			tangent = (s_b2 - s_b1) / (s_t2 - s_t1) + (s_t2 - tf) / (s_t2 - s_t1) * B1p +
			          (tf - s_t1) / (s_t2 - s_t1) * B2p;
		}
		...
		private void CalculateCatmullRomPositionFast(ref Vector3 position, double t, int i)
		{
			...
			if (points.Length > 1)
			{
				position = 0.5f * (2f * s_p[1] + (-s_p[0] + s_p[2]) * t1
				                               + (2f * s_p[0] - 5f * s_p[1] + 4f * s_p[2] - s_p[3]) * t2
				                               + (-s_p[0] + 3f * s_p[1] - 3f * s_p[2] + s_p[3]) * t3);
			}
		}
		...
		private void CalculateCatmullRomTangentFast(ref Vector3 tangent, double t, int i)
		{
			...
			if (points.Length > 1)
			{
				tangent = (6 * t2 - 6 * t1) * s_p[1]
				          + (3 * t2 - 4 * t1 + 1) * (s_p[2] - s_p[0]) * 0.5f
				          + (-6 * t2 + 6 * t1) * s_p[2]
				          + (3 * t2 - 2 * t1) * (s_p[3] - s_p[1]) * 0.5f;
			}
		}
		...
		private void ComputeCatPoints(int i)
		{
			...
			if (m_closed)
			{
				...
				s_p[0] = points[p1].position;
				s_p[1] = points[p2].position;
				s_p[2] = points[p3].position;
				s_p[3] = points[p4].position;
			}
			else
			{
				if (p1 < 0)
				{
					s_p[0] = points[0].position;
					s_p[0] += s_p[0] - points[1].position;
				}
				else
				{
					s_p[0] = points[p1].position;
				...
				if (p3 >= points.Length)
				{
					s_p[2] = points[points.Length - 1].position;
					Vector3 pos = s_p[2];
					s_p[2] += s_p[2] - points[points.Length - 2].position;
					s_p[3] = s_p[2] + (s_p[2] - pos);
				}
				else
				{
					s_p[2] = points[p3].position;
					if (p4 >= points.Length)
					{
						s_p[3] = s_p[2] + (s_p[2] - points[p3 - 1].position);
					}
					else
					{
						s_p[3] = points[p4].position;
