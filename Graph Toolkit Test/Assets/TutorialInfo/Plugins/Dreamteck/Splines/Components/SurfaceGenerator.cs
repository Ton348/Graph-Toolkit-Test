...
namespace Dreamteck.Splines
{
	...
	public class SurfaceGenerator : MeshGenerator
	{
		...
		[SerializeField]
		[HideInInspector]
		private float m_sideUvRotation;
		...
		public void Generate()
		{
			...
			for (var i = 1; i < m_projectedVerts.Length; i++)
			{
				if (min.x < m_projectedVerts[i].x)
				{
					min.x = m_projectedVerts[i].x;
				}

				if (min.y < m_projectedVerts[i].y)
				{
					min.y = m_projectedVerts[i].y;
				}

				if (max.x > m_projectedVerts[i].x)
				{
					max.x = m_projectedVerts[i].x;
				}

				if (max.y > m_projectedVerts[i].y)
				{
					max.y = m_projectedVerts[i].y;
				}
			}

			for (var i = 0; i < m_projectedVerts.Length; i++)
			{
				tsMesh.uv[i].x = Mathf.InverseLerp(max.x, min.x, m_projectedVerts[i].x) * uvScale.x - uvScale.x * 0.5f +
				                 uvOffset.x + 0.5f;
				tsMesh.uv[i].y = Mathf.InverseLerp(min.y, max.y, m_projectedVerts[i].y) * uvScale.y - uvScale.y * 0.5f +
				                 uvOffset.y + 0.5f;
			...
			if (pathExtrude)
			{
				...
				for (var i = 0; i < surfaceVertexCount; i++)
				{
					...
					tsMesh.vertices[i + surfaceVertexCount] = m_extrudeResults[0].position +
					                                          (m_extrudeResults[0].rotation * m_identityVertices[i] +
					                                           off) + vertexOffset;
					...
					tsMesh.vertices[i] = m_extrudeResults[m_extrudeResults.Length - 1].position +
					                     (m_extrudeResults[m_extrudeResults.Length - 1].rotation *
						                     m_identityVertices[i] + off) + vertexOffset;
				...
				//Add wall vertices
				var totalLength = 0f;
				for (var i = 0; i < m_extrudeResults.Length; i++)
				{
					if (m_uniformUvs && i > 0)
					{
						totalLength += Vector3.Distance(m_extrudeResults[i].position, m_extrudeResults[i - 1].position);
					}

					int startIndex = surfaceVertexCount * 2 + i * sampleCount;
					for (var n = 0; n < m_identityVertices.Length; n++)
					{
						Vector3 vertexOffset = TransformOffset(m_extrudeResults[i], m_extrudeOffset);
						tsMesh.vertices[startIndex + n] = m_extrudeResults[i].position +
						                                  (m_extrudeResults[i].rotation * m_identityVertices[n] + off) +
						                                  vertexOffset;
						tsMesh.normals[startIndex + n] = m_extrudeResults[i].rotation * m_identityNormals[n];
						if (m_uniformUvs)
						{
							tsMesh.uv[startIndex + n] =
								new Vector2(
									(float)n / (m_identityVertices.Length - 1) * m_sideUvScale.x + m_sideUvOffset.x,
									totalLength * m_sideUvScale.y + m_sideUvOffset.y);
						}
						else
						{
							tsMesh.uv[startIndex + n] =
								new Vector2(
									(float)n / (m_identityVertices.Length - 1) * m_sideUvScale.x + m_sideUvOffset.x,
									(float)i / (m_extrudeResults.Length - 1) * m_sideUvScale.y + m_sideUvOffset.y);
						}

						if (m_sideUvRotation != 0f)
						{
							tsMesh.uv[startIndex + n] = Quaternion.AngleAxis(m_sideUvRotation, Vector3.forward) *
							                            tsMesh.uv[startIndex + n];
						}
				...
				MeshUtility.GeneratePlaneTriangles(ref m_wallTris, sampleCount - 1, m_extrudeResults.Length, flipSide,
					0, 0, true);
			...
			else if (simpleExtrude)
			{
				...
				//Add wall vertices
				for (var i = 0; i < surfaceVertexCount + 1; i++)
				{
					int index = i;
					...
					tsMesh.uv[i + surfaceVertexCount * 2] = new Vector2(
						(float)i / (surfaceVertexCount - 1) * m_sideUvScale.x + m_sideUvOffset.x,
						0f + m_sideUvOffset.y);
					if (clockwise)
					{
					...
					if (m_uniformUvs)
					{
						tsMesh.uv[offsetIndex] =
							new Vector2((float)i / surfaceVertexCount * m_sideUvScale.x + m_sideUvOffset.x,
								m_extrude * m_sideUvScale.y + m_sideUvOffset.y);
					}
					else
					{
						tsMesh.uv[offsetIndex] =
							new Vector2((float)i / surfaceVertexCount * m_sideUvScale.x + m_sideUvOffset.x,
								1f * m_sideUvScale.y + m_sideUvOffset.y);
					}

					if (m_sideUvRotation != 0f)
					{
						tsMesh.uv[offsetIndex] = Quaternion.AngleAxis(m_sideUvRotation, Vector3.forward) *
						                         tsMesh.uv[offsetIndex];
					}
		...
		private void GenerateSurfaceTris(bool flip)
		{
			...
			if (flip)
			{
				MeshUtility.FlipTriangles(ref m_surfaceTris);
			}
		}
		...
		private void GetIdentityVerts(Vector3 center, Vector3 normal, bool clockwise)
		{
			...
			for (var i = 0; i < sampleCount; i++)
			{
				...
				m_identityVertices[i] = vertsRotation *
				                        (m_evalResult.position - center + (clockwise ? -right : right) * m_expand);
		...
		private void GetProjectedVertices(int count, out Vector3 center, out Vector3 normal)
		{
			...
			if (m_projectedVerts.Length != count)
			{
				m_projectedVerts = new Vector2[count];
			}
			...
			for (var i = 0; i < count; i++)
			{
				...
				if (Vector3.Dot(point, right) < 0.0f)
				{
					projectionPointX *= -1f;
				}

				float projectionPointY = Vector3.Project(point, up).magnitude;
				if (Vector3.Dot(point, up) < 0.0f)
				{
					projectionPointY *= -1f;
				}
