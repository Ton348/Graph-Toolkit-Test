...
namespace Dreamteck.Splines
{
	...
	public class SplineMorph : MonoBehaviour
	{
		...
		private void RunUpdate()
		{
			...
			switch (cycleMode)
			{
				...
				case CycleMode.Loop:
					if (m_cycleValue > 1f)
					{
						m_cycleValue -= Mathf.Floor(m_cycleValue);
					}
		...
		{
			if (index < 0 || index >= m_channels.Length)
			{
				return;
			}

			var newStates = new Channel[m_channels.Length - 1];
			for (var i = 0; i < m_channels.Length; i++)
			{
				if (i == index)
				{
				...
				if (i < index)
				{
					newStates[i] = m_channels[i];
				}
				else if (i >= index)
				{
					newStates[i - 1] = m_channels[i];
				}
			}
		...
		public void UpdateMorph()
		{
			...
			for (var i = 0; i < m_channels.Length; i++)
			{
				for (var j = 0; j < m_points.Length; j++)
				{
					if (i == 0)
					{
					...
					{
						m_points[j].position += (m_channels[i].points[j].position - m_channels[0].points[j].position) *
						                        percent;
						m_points[j].tangent += (m_channels[i].points[j].tangent - m_channels[0].points[j].tangent) *
						                       percent;
						m_points[j].tangent2 += (m_channels[i].points[j].tangent2 - m_channels[0].points[j].tangent2) *
						                        percent;
						m_points[j].normal +=
							(m_channels[i].points[j].normal - m_channels[0].points[j].normal) * percent;
					}
					else
					{
						m_points[j].position = Vector3.Slerp(m_points[j].position,
							m_points[j].position +
							(m_channels[i].points[j].position - m_channels[0].points[j].position), percent);
						m_points[j].tangent = Vector3.Slerp(m_points[j].tangent,
							m_points[j].tangent + (m_channels[i].points[j].tangent - m_channels[0].points[j].tangent),
							percent);
						m_points[j].tangent2 = Vector3.Slerp(m_points[j].tangent2,
							m_points[j].tangent2 +
							(m_channels[i].points[j].tangent2 - m_channels[0].points[j].tangent2), percent);
						m_points[j].normal = Vector3.Slerp(m_points[j].normal,
							m_points[j].normal + (m_channels[i].points[j].normal - m_channels[0].points[j].normal),
							percent);
					}
					...
					if (m_points[j].type == SplinePoint.Type.SmoothMirrored)
					{
						m_points[j].type = m_channels[i].points[j].type;
					}
					else if (m_points[j].type == SplinePoint.Type.SmoothFree)
					{
						if (m_channels[i].points[j].type == SplinePoint.Type.Broken)
						{
							m_points[j].type = SplinePoint.Type.Broken;
						}
					}
				}
			}
