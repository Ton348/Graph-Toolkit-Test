...
namespace Dreamteck.Splines
{
	...
	public class PolygonColliderGenerator : SplineUser
	{
		...
		private void GeneratePath()
		{
			...
			if (m_vertices.Length != vertexCount)
			{
			...
			for (var i = 0; i < sampleCount; i++)
			{
				...
				Vector2 right = new Vector2(-m_evalResult.forward.y, m_evalResult.forward.x).normalized *
				                m_evalResult.size;
				m_vertices[i] = new Vector2(m_evalResult.position.x, m_evalResult.position.y) + right * size * 0.5f +
				                right * offset;
				m_vertices[sampleCount + (sampleCount - 1) - i] =
					new Vector2(m_evalResult.position.x, m_evalResult.position.y) - right * size * 0.5f +
					right * offset;
			}
		}
