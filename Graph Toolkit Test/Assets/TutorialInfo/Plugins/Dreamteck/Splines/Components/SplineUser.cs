...
namespace Dreamteck.Splines
{
	...
	public class SplineUser : MonoBehaviour, ISerializationCallbackReceiver, ISampleModifier
	{
		...
		[SerializeField]
		[HideInInspector]
		protected RotationModifier m_rotationModifier = new();

		[SerializeField]
		[HideInInspector]
		protected OffsetModifier m_offsetModifier = new();

		[SerializeField]
		[HideInInspector]
		protected ColorModifier m_colorModifier = new();

		[SerializeField]
		[HideInInspector]
		protected SizeModifier m_sizeModifier = new();

		[SerializeField]
		[HideInInspector]
		private SplineSample m_clipFromSample, m_clipToSample;

		[SerializeField]
		[HideInInspector]
		private bool m_loopSamples;
		...
		private bool m_rebuild, m_getSamples, m_postBuild;
		...
		public double span
		{
			get
			{
				if (samplesAreLooped)
				{
					return 1.0 - m_clipFrom + m_clipTo;
				}
		...
		public bool samplesAreLooped => m_loopSamples && m_clipFrom >= m_clipTo;

		public RotationModifier rotationModifier => m_rotationModifier;

		public OffsetModifier offsetModifier => m_offsetModifier;

		public ColorModifier colorModifier => m_colorModifier;

		public SizeModifier sizeModifier => m_sizeModifier;
		...
		{
			var clip = false;
			if (m_clipFrom != m_animClipFrom || m_clipTo != m_animClipTo)
			{
				clip = true;
			}
		...
		private void RunUpdate()
		{
			...
			if (m_rebuild)
			{
				...
				{
					if (m_getSamples || m_spline.sampleMode == SplineComputer.SampleMode.Optimized)
					{
						GetSamples();
					}
		...
		private void GetSamples()
		{
			...
			int start, end;
			sampleCount = m_sampleCollection.GetClippedSampleCount(m_clipFrom, m_clipTo, out start, out end);
			double lerp;
		...
		public void ClipPercent(ref double percent)
		{
			...
			if (samplesAreLooped)
			{
				...
				else
				{
					...
					if (Dmath.InverseLerp(clipTo, clipFrom, percent) < 0.5)
					{
						percent = 1.0;
					}
					else
		...
		public double Travel(double start, float distance, Spline.Direction direction, out float moved)
		{
			...
			double result =
				m_sampleCollection.Travel(UnclipPercent(start), distance, direction, out moved, clipFrom, clipTo);
			...
			out float moved)
		{
			...
			double result = m_sampleCollection.TravelWithOffset(UnclipPercent(start), distance, direction, offset,
		...
		public virtual void Project(Vector3 position, ref SplineSample result, double from = 0.0, double to = 1.0)
		{
			...
			m_sampleCollection.Project(position, m_spline.pointCount, ref result, UnclipPercent(from),
				UnclipPercent(to), this);
