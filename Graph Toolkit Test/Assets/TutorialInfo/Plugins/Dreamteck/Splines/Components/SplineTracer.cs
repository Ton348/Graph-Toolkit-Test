...
namespace Dreamteck.Splines
{
	public class SplineTracer : SplineUser
	{
		...
		[SerializeField]
		[HideInInspector]
		protected bool m_dontLerpDirection;
		...
		[SerializeField]
		[HideInInspector]
		protected TransformModule m_motion;
		...
		[SerializeField]
		[HideInInspector]
		protected Rigidbody2D m_targetRigidbody2D;
		...
		[SerializeField]
		[HideInInspector]
		protected SplineSample m_result;

		private readonly List<NodeConnection> m_nodeConnectionQueue = new();
		private int m_addTriggerIndex;
		...
		/// </summary>
		public SplineSample result => m_result;

		public bool dontLerpDirection
		{
			get => m_dontLerpDirection;
			set
			{
		...
		{
			double lastPercent = m_result.percent;
			Evaluate(Travel(0.0, distance), ref m_result);
			ApplyMotion();
			if (checkTriggers)
			{
			...
			if (fromPoint != toPoint)
			{
				if (m_direction == Spline.Direction.Forward)
				{
					for (int i = fromPoint + 1; i <= toPoint; i++)
					{
						...
						if (junction != null)
						{
							m_nodeConnectionQueue.Add(junction);
						}
					}
				}
				else
		...
		protected void CheckTriggers(double from, double to)
		{
			...
			UnclipPercent(ref from);
			UnclipPercent(ref to);
			if (triggerGroup < 0 || triggerGroup >= spline.triggerGroups.Length)
			{
				return;
			}

			for (var i = 0; i < spline.triggerGroups[triggerGroup].triggers.Length; i++)
			{
				if (spline.triggerGroups[triggerGroup].triggers[i] == null)
				{
					continue;
				}
...
#endif
			for (var i = 0; i < m_addTriggerIndex; i++)
			{
		...
		private void AddTriggerToQueue(SplineTrigger trigger)
		{
			...
			{
				var newQueue = new SplineTrigger[m_triggerInvokeQueue.Length +
				                                 spline.triggerGroups[triggerGroup].triggers.Length];
