...
namespace Dreamteck.Splines
{
	...
	public class SplineFollower : SplineTracer
	{
		...
		/// <summary>
		///     If the follow mode is set to Uniform and there is an added offset in the motion panel, this will presserve the
		///     uniformity of the follow speed
		/// </summary>
		[HideInInspector]
		...
		[SerializeField]
		[HideInInspector]
		private FollowerSpeedModifier m_speedModifier = new();

		[SerializeField]
		[HideInInspector]
		private FloatEvent m_unityOnEndReached;

		[SerializeField]
		[HideInInspector]
		private FloatEvent m_unityOnBeginningReached;
		...
		public float followSpeed
		{
			...
			set
			{
				if (m_followSpeed != value)
				{
					...
					if (Mathf.Approximately(m_followSpeed, 0f))
					{
						return;
					}
		...
		public FollowerSpeedModifier speedModifier => m_speedModifier;
		...
		private void Follow()
		{
			switch (followMode)
			{
				...
				case FollowMode.Time:
					if (m_followDuration == 0.0)
					{
						Move(0.0);
					}
					else
					{
						Move((double)Time.deltaTime / m_followDuration);
					}
		...
		public override void SetDistance(float distance, bool checkTriggers = false, bool handleJunctions = false)
		{
			...
			if (samplesAreLooped && clipFrom == clipTo && distance > 0f && m_lastClippedPercent == 0.0)
			{
				m_lastClippedPercent = 1.0;
			}
		...
		public void Move(double percent)
		{
			...
			if (wrapMode == Wrap.Default && m_lastClippedPercent >= 1.0 && startPercent == 0.0)
			{
				startPercent = 1.0;
			}
