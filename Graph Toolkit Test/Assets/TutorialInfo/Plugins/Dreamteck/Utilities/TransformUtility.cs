...
namespace Dreamteck
{
	public static class TransformUtility
	{
		...
		public static void MergeBoundsRecursively(
			this Transform rootParent,
			Transform tr,
			ref Bounds bounds,
			string nameToIgnore = null)
		{
			foreach (Transform child in tr)
			{
				...
				Vector3 min = child.TransformPoint(meshFilter.sharedMesh.bounds.min);
				Vector3 max = child.TransformPoint(meshFilter.sharedMesh.bounds.max);
		...
		public static void DestroyChildren(this Transform src)
		{
			...
			{
				Object.Destroy(src.GetChild(i).gameObject);
			}
		}
