...
namespace Dreamteck
{
	public class MeshUtility
	{
		...
		public static int[] GeneratePlaneTriangles(
			int x,
			int z,
			bool flip,
			...
			bool reallocateArray = false)
		{
			...
			if (reallocateArray && triangles.Length != nbFaces * 6)
			{
				...
				{
					var newTris = new int[startTriangleIndex + nbFaces * 6];
					for (var i = 0; i < startTriangleIndex; i++)
					{
			...
			for (var face = 0; face < nbFaces + z - 2; face++)
			{
				if ((face + 1) % (float)g == 0f && face != 0)
				{
					face++;
				}
		...
		public static void MakeDoublesided(Mesh input)
		{
			...
			for (var i = 0; i < input.subMeshCount; i++)
			{
				submeshes.Add(input.GetTriangles(i));
			}

			var newVertices = new Vector3[vertices.Length * 2];
			var newNormals = new Vector3[normals.Length * 2];
			var newUvs = new Vector2[uvs.Length * 2];
			var newColors = new Color[colors.Length * 2];
			var newTris = new int[triangles.Length * 2];
			...
			for (var i = 0; i < submeshes.Count; i++)
			{
				for (var n = 0; n < submeshes[i].Length; n += 3)
				{
			...
			for (var i = 0; i < newSubmeshes.Count; i++)
			{
				input.SetTriangles(newSubmeshes[i], i);
			}
		}
		...
		public static void MakeDoublesidedHalf(TsMesh input)
		{
			...
			for (var i = 0; i < vertexHalf; i++)
			{
				...
				{
					input.tangents[i + vertexHalf] = input.tangents[i];
				}
				...
				{
					input.uv2[i + vertexHalf] = input.uv2[i];
				}

				if (input.uv3.Length > i)
				{
					input.uv3[i + vertexHalf] = input.uv3[i];
				}

				if (input.uv4.Length > i)
				{
					input.uv4[i + vertexHalf] = input.uv4[i];
				}
		...
		public static void Triangulate(Vector2[] points, ref int[] output)
		{
			...
			var V = new int[pointsLength];
			if (Area(points, pointsLength) > 0)
			{
				for (var v = 0; v < pointsLength; v++)
				{
					V[v] = v;
				}
			}
		...
		public static void FlipFaces(TsMesh input)
		{
			...
			for (var i = 0; i < input.normals.Length; i++)
			{
				input.normals[i] *= -1f;
			}
		}
		...
		{
			var newVertices = new Vector3[input.triangles.Length];
			...
			var newColors = new Color[newVertices.Length];
			var newBoneWeights = new BoneWeight[newVertices.Length];
			...
			for (var i = 0; i < submeshes; i++)
			{
				...
				for (var n = 0; n < submesh.Length; n += 3)
				{
					...
					if (oldNormals.Length > submesh[n + 2])
					{
						if (!keepNormals)
						{
							newNormals[vertIndex] = newNormals[vertIndex + 1] = newNormals[vertIndex + 2] =
								(oldNormals[submesh[n]] + oldNormals[submesh[n + 1]] + oldNormals[submesh[n + 2]])
								.normalized;
						}
						else
						{
					...
					{
						newColors[vertIndex] = newColors[vertIndex + 1] = newColors[vertIndex + 2] =
							(oldColors[submesh[n]] + oldColors[submesh[n + 1]] + oldColors[submesh[n + 2]]) / 3f;
					}
