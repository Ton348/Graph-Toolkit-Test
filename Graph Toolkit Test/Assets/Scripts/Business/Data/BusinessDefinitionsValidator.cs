...
public static class BusinessDefinitionsValidator
{
        ...
        CustomerBehaviorDatabaseData behaviors)
    {
        ...
        {
            foreach (var type in businessTypes.businessTypes)
            {
                ...
                {
                    foreach (var moduleId in type.requiredModules)
                    {
                        if (string.IsNullOrWhiteSpace(moduleId))
                        {
                            Debug.LogError($"[BusinessDefinitions] business type {type.id} has empty module reference.");
                            ok = false;
                            continue;
                        }
                        if (!moduleIds.Contains(moduleId))
                        {
                            Debug.LogError($"[BusinessDefinitions] business type {type.id} references missing module: {moduleId}");
                            ok = false;
                        }
                    }
                }
        ...
        else
        {
            ...
            foreach (var supplier in suppliers.suppliers)
            {
                ...
                {
                    Debug.LogError($"[BusinessDefinitions] supplier {supplier.id} minDeliveryAmount > maxDeliveryAmount.");
                    ok = false;
                }
            }
        }
        ...
        else
        {
            ...
            foreach (var behavior in behaviors.behaviors)
            {
                ...
                {
                    Debug.LogError($"[BusinessDefinitions] duplicate customer behavior for businessTypeId: {behavior.businessTypeId}");
                    ok = false;
                }
                if (!businessTypeIds.Contains(behavior.businessTypeId))
                {
                    Debug.LogError($"[BusinessDefinitions] customer behavior references unknown businessTypeId: {behavior.businessTypeId}");
                    ok = false;
                }
                if (behavior.arrivalRatePerHour < 0)
                {
                    Debug.LogError($"[BusinessDefinitions] customer behavior {behavior.businessTypeId} has negative arrivalRatePerHour.");
                    ok = false;
                }
            }
        }
