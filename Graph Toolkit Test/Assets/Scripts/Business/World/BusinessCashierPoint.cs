...
public class BusinessCashierPoint : MonoBehaviour
{
    ...
    private void ApplyMultiplier(bool active)
    {
        ...
        var simulation = worldRuntime.GetSimulationService();
        if (simulation == null)
        {
            return;
        }
        ...
        BusinessDebugLog.Log($"[BusinessWorld] Cashier point {(active ? "entered" : "exited")} lotId='{worldRuntime.lotId}' multiplier={(active ? cashierMultiplier : 1f)}");
    }
}
