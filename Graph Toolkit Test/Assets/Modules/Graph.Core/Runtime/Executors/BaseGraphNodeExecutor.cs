...
namespace Graph.Core.Runtime.Executors
{
	public abstract class BaseGraphNodeExecutor<TNode> : IGraphNodeExecutor where TNode : BaseGraphNode
	{
			...
			CancellationToken cancellationToken)
		{
			if (context == null)
			{
				return UniTask.FromResult(
					GraphNodeExecutionResult.Fault($"Graph execution context is null for executor {GetType().Name}."));
			}

			if (node == null)
			{
				return UniTask.FromResult(
					GraphNodeExecutionResult.Fault($"Graph node is null for executor {GetType().Name}."));
			}
			...
			if (node is not TNode typedNode)
			{
				...
				var message =
					$"Node type mismatch for executor {GetType().Name}. Expected {typeof(TNode).Name}, got {actualType}.";
		...
		protected abstract UniTask<GraphNodeExecutionResult> ExecuteTypedAsync(
			TNode node,
			GraphExecutionContext context,
			CancellationToken cancellationToken);
	}
