...
public sealed class RequestAssignSupplierNodeValidator : IGameGraphNodeValidator
{
	public Type NodeType => typeof(RequestAssignSupplierNode);

	public bool Validate(GameGraphNode node, GameGraphValidationResult result)
	{
		if (!GameGraphBusinessValidatorHelpers.ValidateType(node, result, out RequestAssignSupplierNode typedNode))
		{
			return false;
		}
		...
		valid &= GameGraphValidationHelpers.ValidateRequiredString(typedNode.supplierId, typedNode, nameof(typedNode.supplierId), result);
		return valid;
	}
}
